...
// CRC-32/ISO-HDLC of the grammar file: 6530be1c
...
#[derive(Debug, Clone)]
pub struct ID_SIMPLE;
#[derive(Debug, Clone)]
pub struct ID_ESCAPED;
...
mod peginator_generated {
    ...
    mod IdentifierNode_impl {
        ...
        pub fn parse<'a, TT: ParseTracer>(
            ...
            ChoiceHelper::new(state)
                .choice(|state| {
                    parse_Whitespace(state, &mut *global)
                        .and_then(|ParseOk { state, .. }| parse_ID_SIMPLE(state, &mut *global))
                        .discard_result()
                })
                .choice(|state| {
                    parse_Whitespace(state, &mut *global)
                        .and_then(|ParseOk { state, .. }| parse_ID_ESCAPED(state, &mut *global))
                        .discard_result()
                })
                .end()
        }
    ...
    #[inline]
    pub(super) fn parse_ID_SIMPLE<'a, TT: ParseTracer>(
        state: ParseState<'a>,
        global: &mut ParseGlobal<TT, ParseCache<'a>, ()>,
    ) -> ParseResult<'a, ID_SIMPLE> {
        global.tracer.print_trace_start(&state, "ID_SIMPLE");
        let result = {
            let result = ID_SIMPLE_impl::parse(state, global)?.map(|r| super::ID_SIMPLE {});
            Ok(result)
        };
        global.tracer.print_trace_result(&result);
        result
    ...
    mod ID_ESCAPED_impl {
        ...
        mod part_1 {
            ...
            pub fn parse<'a, TT: ParseTracer>(
                ...
                let mut state = state;
                loop {
                    match parse_char(state.clone(), &mut *global).discard_result() {
                        Ok(ParseOk { result: __result, state: new_state, .. }) => {
                            state = new_state;
                        }
                        Err(err) => {
    ...
    #[inline]
    pub(super) fn parse_ID_ESCAPED<'a, TT: ParseTracer>(
        state: ParseState<'a>,
        global: &mut ParseGlobal<TT, ParseCache<'a>, ()>,
    ) -> ParseResult<'a, ID_ESCAPED> {
        global.tracer.print_trace_start(&state, "ID_ESCAPED");
        let result = {
            let result = ID_ESCAPED_impl::parse(state, global)?.map(|r| super::ID_ESCAPED {});
            Ok(result)
        };
        global.tracer.print_trace_result(&result);
        result
