        ...
        mod rules {
            ...
            pub mod visible {
                                            ...
                                            state
                                                .sequence(|state| {
                                                    self::Comma(state)
                                                        ...
                                                        .and_then(|state| self::key_value(state))
                                                })
                                                .and_then(|state| {
                                                    state.repeat(|state| {
                                                        state.sequence(|state| {
                ...
                pub fn index(
                    ...
                    state.rule(Rule::index, |state| {
                        self::index_step(state)
                            .or_else(|state| self::index_range(state))
                            .or_else(|state| self::index_element(state))
                    })
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                ...
                pub fn key_valid(
                    ...
                    state.atomic(::pest::Atomicity::NonAtomic, |state| {
                        state.rule(Rule::key_valid, |state| {
                            self::Integer(state)
                                .or_else(|state| self::SYMBOL(state))
                                .or_else(|state| self::String(state))
                        })
                ...
                pub fn element(
                    ...
                    state.atomic(::pest::Atomicity::NonAtomic, |state| {
                        state.rule(Rule::element, |state| {
                            self::data(state)
                                .or_else(|state| self::expr(state))
                                .or_else(|state| self::statement(state))
                        })
                    })
                }
                #[inline]
                ...
                pub fn index_element(
                    ...
                    state.atomic(::pest::Atomicity::NonAtomic, |state| {
                        state.rule(Rule::index_element, |state| self::element(state))
                    })
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                ...
                pub fn Symbol(
                    ...
                    state.atomic(::pest::Atomicity::CompoundAtomic, |state| {
                        state.rule(Rule::Symbol, |state| {
                            self::maybeSymbol(state)
                                .or_else(|state| self::mustSymbol(state))
                                .or_else(|state| self::SYMBOL(state))
                        })
                    })
                }
                        ...
                        state.rule(Rule::mustSymbol, |state| {
                            state.sequence(|state| {
                                self::namespace(state)
                                    .and_then(|state| {
                                        state.sequence(|state| {
                                            self::Dot(state).and_then(|state| self::SYMBOL(state))
                                        })
                                    })
                                    .and_then(|state| {
                                        state.repeat(|state| {
